...
class ThaiTokenizer:
    def __init__(self):
        ...
        # English Alphabets
        self.english_chars = [chr(x) for x in range(ord('a'), ord('z') + 1)] + [chr(x) for x in range(ord('A'), ord('Z') + 1)]
        # Basic punctuation and space
        self.extras = [" ", ".", ",", "!", "?", "(", ")", "-"]

        # Combine all to create vocabulary
        self.vocab = self.special_tokens + self.consonants + self.vowels_tones + self.thai_digits + self.digits + self.english_chars + self.extras
